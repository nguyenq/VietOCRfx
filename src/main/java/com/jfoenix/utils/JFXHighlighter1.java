...
public class JFXHighlighter1 {
     ...
     */
    public synchronized void highlight(Parent pane, List<IndexRange> ranges) {
        if (this.parent != null && !boxes.isEmpty()) {
            clear();
        }
        ...
        for (Node node : nodes) {
            Text text = ((Text) node);
            if (node.isVisible()) {
                ArrayList<Bounds> boundingBoxes = getMatchingBounds(ranges, text);
                ArrayList<Rectangle> rectangles = new ArrayList<>();
                for (Bounds boundingBox : boundingBoxes) {
                    HighLightRectangle rect = new HighLightRectangle(text);
                    rect.setCacheHint(CacheHint.SPEED);
...
//            highlight(parent, ranges);
    ...
    private ArrayList<Bounds> getMatchingBounds(List<IndexRange> ranges, Text text) {
        // find local text bounds in parent
        Bounds textBounds = parent.sceneToLocal(text.localToScene(text.getBoundsInLocal()));
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            for (IndexRange range : ranges) {
                ...
                Text temp = null;
                try {
                    temp = new Text(lineText.substring(range.getStart() - line.getStart(), range.getEnd() - line.getStart()));
                ...
                temp.applyCss();
                double width = temp.getLayoutBounds().getWidth();
                temp.setText(lineText.substring(0, range.getEnd() - line.getStart()));
                temp.applyCss();
                double maxX = temp.getLayoutBounds().getMaxX();
                double startX = maxX - width;
